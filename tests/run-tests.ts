...
// Test 4: Mock and live services share same schema contract (overview example)
resetEnv();
process.env.NEXT_PUBLIC_APP_ENV = "local";
process.env.NEXT_PUBLIC_USE_MOCK_DATA = "true";
{
  ...
  // live service requires client; expect it to return empty contract without thrown schema error
  const liveRes = live.getOverviewData({
    userId: "user-1",
    supabaseClient: {},
  });
  schema.OverviewResponseSchema.parse(liveRes);
}
